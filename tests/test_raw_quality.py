...
def load_raw_data():
    ...
    try:
        data = {
            ...
            "cursos": duckdb.sql(f"SELECT * FROM read_parquet('{BASE_DIR}/Dim_Cursos.parquet')").df(),
            ...
            "Absenteismo": duckdb.sql(f"SELECT * FROM read_parquet('{BASE_DIR}/Fato_Absenteismo_Historico.parquet')").df(),
            "LMS": duckdb.sql(f"SELECT * FROM read_parquet('{BASE_DIR}/Fato_Engajamento_LMS.parquet')").df(),
            "beneficios_custo": duckdb.sql(f"SELECT * FROM read_parquet('{BASE_DIR}/Fato_Custo_Beneficios.parquet')").df(),
            "ATS": duckdb.sql(f"SELECT * FROM read_parquet('{BASE_DIR}/Fato_ATS_Funil.parquet')").df(),
            "beneficios_vidas": duckdb.sql(f"SELECT * FROM read_parquet('{BASE_DIR}/Fato_Vidas_Beneficios.parquet')").df()
        }
...
def test_arquivos_existem():
    ...
    arquivos_obrigatorios = [
        ...
        "Fato_Custo_Beneficios.parquet",
        "Fato_Engajamento_LMS.parquet",
        "Fato_ATS_Funil.parquet",
        "Fato_Absenteismo_Historico.parquet",
        "Fato_Vidas_Beneficios.parquet"
    ]
...
def teste_integridade_temporal_absenteismo(load_raw_data):
    """Garante que nenhum afastamento termine antes de começar"""
    df_abs = load_raw_data["Absenteismo"]
    
    data_inicio = pd.to_datetime(df_abs["data_inicio"])
    data_fim = pd.to_datetime(df_abs["data_fim"])
    incosistentes = df_abs[data_fim < data_inicio]
    assert len(incosistentes) == 0, f"Encontrados {len(incosistentes)} afastamentos com a `data_fim` menor que a `data_inicio`"
    
def test_sql_recrutamento_ats(load_raw_data):
    """Garante que o tempo de recrutamento da vaga (SLA) não seja negativo"""
    df_ats = load_raw_data["ATS"]
    assert(df_ats["sla_dias_fechamento"] >= 0).all(), "Existem requisições fechadas com o SLA de tempo negativo."
    assert(df_ats["qtd_aplicacoes"] >= df_ats["qtd_triagem"]).all(), "Triagem maior que o topo do fúnil de aplicações."

def test_coerencia_faturamento_beneficios(load_raw_data):
    """Verifica se não existem beneficios ou idades impossíveis de faturar"""
    df_vidas = load_raw_data["beneficios_vidas"]
    assert(df_vidas["idade_vigente"] >= 0).all(), "Beneficiário faturado com idade negativa"
    assert(df_vidas["custo_saude"] >= 0).all(), "Faturamento de Saúde com valor negativo."
    
def test_regras_estado_lms(load_raw_data):
    """Valida a lógica transacional do status acadêmico (Concluído, Abandono, Reprovado)"""
    df_lms = load_raw_data["LMS"]
    abandonos = df_lms[df_lms["status"] == "Abandonado"]
    assert abandonos["nota_final"].isnull().all(), "Cursos Abandonados não devem possuir nota"
    assert abandonos["data_conclusao"].isnull().all(), "Cursos Abandonados não devem possuir data de conclusão"
    
    concluidos = df_lms[df_lms["status"].isin(["Concluído", "Reprovado"])]
    assert concluidos["nota_final"].notnull().all(), "Cursos finalizados obrigatoriamente devem possuir nota."
