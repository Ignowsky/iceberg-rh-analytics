...
def test_integridade_contratos(load_bronze_data):
    """
    Valida se a tabela de contratos possui registros e as colunas essenciais.
    """
    df_contratos = load_bronze_data["contratos"]
...
def test_sem_salario_negativos_ou_nulos(load_bronze_data):
    """
    ...
    df_snapshot = load_bronze_data["snapshot"]
...
def test_integridade_referencial_snapshot(load_bronze_data):
    """
...
def test_snapshot_faixa_competencia(load_bronze_data):
    """
    Garante que as competências do snapshot respeitam o horizonte histórico simulado
    """
    
    df_snapshot = load_bronze_data["snapshot"]
    df_ponto_mensal = load_bronze_data["Ponto"]
...
def test_dominio_pesquisa_clima(load_bronze_data):
    """
    Valida se as notas e grupos da pesquisa de clima estão dentro dos domínios aceitaveis
    """
    df_clima = load_bronze_data["Pesquisa_Clima"]
...
def test_sanidade_ponto_mensal(load_bronze_data):
    """
    Valida a consistência das marcações de ponto e banco de horas.
    """
    df_ponto = load_bronze_data["Ponto"]
